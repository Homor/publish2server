...
function parseHook(config) {
    ...
    return new Promise(async (resolve) => {
        ...
        if (open && open.url) {
            console.log(chalk.yellow(`即将打开：${open.url}`));
        ...
        if (request && request.appkey) {
            ...
            if (fetchResult.code == 1000) {
                ...
                console.log(chalk.red(`请求接口失败!! ${options.hostname}:${options.port}/${options.path}?${options.key}=${options.appkey}`));
            }
        }
        resolve(null);
    });
...
async function run(config) {
    ...
    try {
        ...
        console.log(chalk.red(`${currentData.name}(${currentData.host}),发布失败`));
        if (error.level) {
            console.log(chalk.red(error.level));
        }
        return closeInput();
    }
    ...
    try {
        ...
        console.log(chalk.red(`${currentData.api_host}/${currentData.api_path},刷新CDN失败`));
        if (error.level) {
            console.log(chalk.red(error.level));
        }
    }
